...
BookInstanceSchema.statics.getAllBookStatuses = async function (): Promise<
  string[]
> {
  const listBookInstances: IBookInstance[] = await BookInstance.find({
    status: { $eq: "Available" },
  }).populate("book");
 ...
 */
BookInstanceSchema.statics.getBookInstanceCount = async function (
  filter?: FilterQuery<IBookInstance>
): Promise<number> {
 ...
 */
const BookInstance = mongoose.model<IBookInstance, IBookInstanceModel>(
  "BookInstance",
  BookInstanceSchema
);
